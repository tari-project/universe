...
describe('airdropStoreActions', () => {
    ...
    describe('calculateBalanceSummaryWithProgram', () => {
        interface Tranche {
            id: string;
            amount: number;
            claimed: boolean;
        ...
        interface TrancheStatusWithProgram {
            tranches: Tranche[];
        }

        function calculateBalanceSummaryForProgram(
            trancheStatus: TrancheStatusWithProgram,
            program?: string
        ) {
            const filtered = program
                ? trancheStatus.tranches.filter((t) => t.program === program)
                : trancheStatus.tranches;

            const totalXtm = filtered.reduce((sum, t) => sum + t.amount, 0);
            const totalClaimed = filtered
                .filter((t) => t.claimed)
                .reduce((sum, t) => sum + t.amount, 0);

            const now = new Date();
            const totalExpired = filtered
                .filter((t) => !t.claimed && new Date(t.validTo) < now)
                .reduce((sum, t) => sum + t.amount, 0);

            const totalPending = totalXtm - totalClaimed - totalExpired;

            return { totalXtm, totalClaimed, totalPending, totalExpired };
        }
        ...
        const mixedTranches: TrancheStatusWithProgram = {
            tranches: [
        ...
        it('returns all tranches when no program filter', () => {
            const result = calculateBalanceSummaryForProgram(mixedTranches);
            expect(result.totalXtm).toBe(4250);
            expect(result.totalClaimed).toBe(1500);
        });

        it('filters to airdrop program only', () => {
            const result = calculateBalanceSummaryForProgram(mixedTranches, 'airdrop');
            expect(result.totalXtm).toBe(3000);
        ...
        it('returns zeros for unknown program', () => {
            const result = calculateBalanceSummaryForProgram(mixedTranches, 'unknown');
            expect(result.totalXtm).toBe(0);
            expect(result.totalClaimed).toBe(0);
            expect(result.totalPending).toBe(0);
            expect(result.totalExpired).toBe(0);
