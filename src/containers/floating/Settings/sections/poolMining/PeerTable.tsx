...
const headings = ['#', 'Peer ID', 'SHA3X Height', 'Last Ping'];
...
export default function PeerTable({ peers }: { peers: ConnectedPeerInfoExtended[] }) {
    ...
    const peerMarkup = peers.map(({ peer_id, peer_info, last_ping, sha3Diff }, idx) => {
        const count = idx + 2;
        const displayId = truncateMiddle(peer_id, 9);
        const { current_sha3x_height: sha3x_height } = peer_info || {};
