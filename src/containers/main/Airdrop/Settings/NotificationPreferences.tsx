...
const useGetNotificationPreferences = () => {
    const [notificationPreferences, setNotificationPreferences] = useState<MarketingPreferences>();
    ...
    useEffect(() => {
        if (!notificationPreferences && !isFetching) fetchNotificationPreferences();
    }, [fetchNotificationPreferences, isFetching, notificationPreferences]);

    return {
        notificationPreferences,
        fetchPreferences: fetchNotificationPreferences,
        isFetching,
        setMarketingPreferences: setNotificationPreferences,
    };
};
...
export default function NotificationPreferences() {
    ...
    const hendleUpdatePreferences = useUpdateNotificationPreferences();
    ...
    const subscribed = useMemo(() => preferences?.emailEvents?.every((event) => event.subscribed), [preferences]);

    const toggleAllPreference = useCallback(() => {
        if (!preferences?.emailEvents) return;
    ...
    }, [preferences, setMarketingPreferences, subscribed, updatePreferences]);
