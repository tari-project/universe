...
impl CpuMiner {
    ...
    pub async fn start_clythor(
        ...
        let (mut clythor_child, _clythor_status_monitor) =
        ...
        self.watcher_task = Some(tauri::async_runtime::spawn(async move {
            ...
            loop {
                select! {
                      _ = watch_timer.tick() => {
                            if !clythor_child.ping()
                            {
                               warn!(target: LOG_TARGET, "clythor is not running");
                               match clythor_child.stop().await {
                                   Ok(_) => {
                                      info!(target: LOG_TARGET, "clythor exited successfully");
                                   }
                                   Err(e) => {
                                      error!(target: LOG_TARGET, "clythor exited with error: {}", e);
                                      return Err(e)
                                   }
                               }
                               break;
